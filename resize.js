...
// Also downscale responsive prefixes (md:text-7xl -> md:text-6xl, etc.)
const allMappings = {};
for (const [key, value] of Object.entries(mapping)) {
  allMappings[key] = value;
  allMappings[`sm:${key}`] = `sm:${value}`;
  allMappings[`md:${key}`] = `md:${value}`;
  allMappings[`lg:${key}`] = `lg:${value}`;
  allMappings[`xl:${key}`] = `xl:${value}`;
  allMappings[`2xl:${key}`] = `2xl:${value}`;
}
...
files.forEach(file => {
  ...
  content = content.replace(/className=(['"])(.*?)\1|className=\{([\s\S]*?)\}/g, (match, p1, p2, p3) => {
    let cls = p2 || p3;
    if (cls && cls.includes('font-helvetica')) {
      for (const [key, value] of Object.entries(allMappings)) {
        // match exactly e.g. "text-7xl", not "md:text-7xl" if key is "text-7xl"
        // Since key might have backslashes already doubled for literal regex string
        const regex = new RegExp('(?<![\\w-])' + key + '(?![\\w-])', 'g');
        cls = cls.replace(regex, value);
      }
